...
readonly BASE_DIR="$HOME/dusk_container/uv/$APP_NAME"
...
if pgrep -f "$RUNNER_SCRIPT" >/dev/null; then
  ...
  # Notify user
  command -v notify-send >/dev/null && notify-send --app-name="WayClick" "WayClick" "Disabled"
...
INTERACTIVE=true
[[ -n "${WC_KB:-}" ]] && INTERACTIVE=false

notify_user() {
  # Send notification if possible (for keybind feedback)
  if command -v notify-send >/dev/null; then
    notify-send --app-name="WayClick" "WayClick" "$1"
  fi
}
